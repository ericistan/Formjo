  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const details = await Promise.all(
        list.map((a) =>
          fetch(`${import.meta.env.VITE_API_URL}/student/assignments/${a.id}`, {
            headers: { Authorization: `Bearer ${token}` },
  ...
  return (
    <div className="py-8 max-w-3xl mx-auto flex flex-col gap-8 px-4">
      <div className="rounded-xl bg-foreground text-background p-6 flex flex-col gap-1">
        <p className="text-sm uppercase tracking-widest opacity-60">Welcome back</p>
            ...
            {assignments.map((a) => (
              ...
              >
                ...
                <div className="flex flex-col gap-3 p-4">
                  <p className="text-sm text-muted-foreground">{a.coach_name}</p>
                  ...
                  {progress[a.id] && (
                    <div className="flex flex-col gap-1.5">
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Lessons</span>
                        <span>{progress[a.id].done}/{progress[a.id].total} done</span>
                      </div>
